...
df_raw_population = spark.read.csv("/mnt/covidreporting31dl/raw/population", sep=r'\t', header=True)
df_raw_population.show()

# COMMAND ----------

df_raw_population = spark.read.csv("/mnt/covidreporting31dl/raw/population", sep=r'\t', header=True)
#Using indic_de,geo\\time column creating 2 new seperate columns(age_group, country_code)
...
df_raw_population_pivot = spark.sql("SELECT country_code, age_group,percentage_2019,  cast(regexp_replace(percentage_2019, '[a-z]', '') AS decimal(4,2)) AS percentage_2020 FROM raw_population")
df_raw_population_pivot.show()
...
# MAGIC %sql 
# MAGIC
# MAGIC select * from raw_population_pivot
...
df_dim_country = spark.read.csv("/mnt/covidreporting31dl/lookup/dim_country", sep=r',', header=True)
...
df_processed_population.write.format("com.databricks.spark.csv").option("header","true").option("delimiter", ",").mode("overwrite").save("/mnt/covidreporting31dl/processed/population")
